document.write("<style>"+
	...
	".loadingRun{-webkit-animation : loadingAnimate 1.2s infinite;animation : loadingAnimate 1.2s infinite;}#passwordBox{display: none;position: absolute;left: 50%;top:20%;z-index: 2;width: 200px;margin-left: -100px;}#passwordBox input{width: 120px;float: left;}#passwordBox button{width: 60px;float: right;}</style>");
...
LoadingJS.prototype = {
	...
	initHtml : function(){
		...
		passwordBtn.setAttribute("id","submitPass");
		passwordBtn.innerHTML="解锁";
	...
	　　	input.setAttribute("id", 'password') ;
		...
		passwordBox.appendChild(passwordBtn);
		...
		this.instance.appendChild(passwordBox);
		this.bg.appendChild(this.instance);
		document.body.appendChild(this.bg);
		var that=this;
		passwordBtn.onclick=function(){
			if(input.value	===	bookConfig.openTheDoor){
				try{
					localStorage.setItem('bookpass', input.value);
				}catch(e){
					console.log(e)
				}
				bookConfig.openTheDoor='';
				that.destroy()
			}else{
				alert('密码错误')
				input.value=''
	...
	destroy : function(){
		try{
			if(bookConfig.openTheDoor && localStorage.getItem('bookpass')!==bookConfig.openTheDoor){
				$('#passwordBox').show()
				return
			}
		}catch(e){
